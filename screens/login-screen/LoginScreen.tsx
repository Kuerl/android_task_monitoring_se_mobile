...
export default function LoginScreen(
  { form, setForm, after, setAuth }:
  { form: any, setForm: Function, after: boolean, setAuth: Function}
) {
  ...
  return(
    <View style={loginStyles.container} >
      ...
      <View style={loginStyles.signin} >
        <View>
          <InputComponent type={'Username'} state={signin.username} setState={
            (e: string) => setSignIn(prevstate => ({...prevstate, username: e}))
          } setDisplay={(e: any) => {setDisplay(e)}} />
          <InputComponent type={'Password'} state={signin.password} setState={
            (e: string) => setSignIn(prevstate => ({...prevstate, password: e}))
          } setDisplay={(e: any) => {setDisplay(e)}} />
        </View>
      ...
      <View style={loginStyles.none} >
        <View>
          {
            after ?
              <Text style={loginStyles.none__check}>Create account successfully. You can login now.</Text> :
              ((signin.username.length < 6 || signin.username.length > 20) && display.edited === true) ?
                <Text style={loginStyles.none__check}>Notvalid input username</Text> : 
                resError.effect === true ?
                  <Text style={loginStyles.none__check}>{resError.status}</Text> :
                  <></>
          }
        </View>
      </View>
